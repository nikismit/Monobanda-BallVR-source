...
public class AudioPitch_MenuPlayer1 : MonoBehaviour {
//	public GUIText noteText;
    public static int _currentPitch;
    public int Mic_Input;
...
/*
	...
	void Start () {
		...
		selectedDevice = Microphone.devices[Mic_Input].ToString();
    ...
    public void reSetupMic(){
      Mic_Input = dropdown.value;
      selectedDevice = Microphone.devices[Mic_Input].ToString();
          selectedMic = selectedDevice;
          micSelected = true;
  		GetMicCaps();
	...
	void FixedUpdate () {
    PlayerPrefs.SetInt("Player1MicIndex",Mic_Input);
		if (listening) {
